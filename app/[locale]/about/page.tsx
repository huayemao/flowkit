...
import { getAboutContent } from "@/lib/about/mdx"
...
export async function generateMetadata({ params }: AboutPageProps) {
  ...
  const mdxContent = await getAboutContent(locale)
  
  if (!mdxContent) {
    return {
      title: "About - utities.online",
    }
  }
        ...
        <header className="mb-16">
          <div className="inline-block border-2 border-foreground px-4 py-1 mb-8">
            <span className="text-xs font-mono font-bold uppercase tracking-[0.3em]">
              {t('nav_about')} utities.online
            </span>
          </div>
          <h1 className="text-5xl md:text-7xl font-serif font-bold tracking-tighter mb-8 leading-[0.9]">
            {t('nav_about')}
          </h1>
          <p className="text-xl md:text-2xl font-serif italic text-muted-foreground leading-tight">
