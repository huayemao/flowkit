...
import { renderIconToDOM } from "./icon-renderer";
...
export const LogoPreview: React.FC<LogoPreviewProps> = ({
  ...
  const generateLogoPreview = () => {
    ...
    if (config.isGradient && !config.isCustomColor) {
      previewElement.className = `rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 ease-in-out bg-gradient-to-br ${
        gradientStyles[config.gradientStyle]
      }`;
    } else {
      previewElement.className = `rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 ease-in-out`;
      previewElement.style.backgroundColor = config.isCustomColor
    ...
    // 渲染图标到预览容器
    renderIconToDOM(config, previewElement);
