...
export const exportAsPNG = async (
  ...
  try {
    ...
    const iconSize =
      Math.min(size.width, size.height) * (1 - 2 * config.iconMarginRatio);

    // 渲染图标
    const iconSvg = getIconAsSVGString(config, iconSize);
    ...
    if (iconSvg) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        img.onerror = () => {
          console.error("无法加载图标 SVG");
          reject(new Error("无法加载图标 SVG"));
        };
        img.src = dataUrl;
      });
    } else {
      console.error("无法获取图标 SVG 字符串");
    }
        ...
        >
          ...
          {config.isGradient && !config.isCustomColor ? (
            ...
            <rect
              ...
              rx="12"
              fill={
                config.isCustomColor ? config.customBackgroundColor : "#4F46E5"
              }
            />
          )}

          {renderIconComponent(config,'export')}
          ...
          reader.onload = (e) => {
            try {
              ...
              setConfig((prev) => ({
                ...prev,
                gradientStyle:
                  template.gradientStyle &&
                  typeof template.gradientStyle === "string" &&
                  gradientStyles[template.gradientStyle as GradientStyle]
                    ? (template.gradientStyle as GradientStyle)
                    : prev.gradientStyle,
                icon:
                  template.icon && validIcons.includes(template.icon)
                    ? template.icon
                    : prev.icon,
                ...
                isCustomColor:
                  typeof template.isCustomColor === "boolean"
                    ? template.isCustomColor
                    : prev.isCustomColor,
                customBackgroundColor:
                  template.customBackgroundColor || prev.customBackgroundColor,
                isGradient:
                  typeof template.isGradient === "boolean"
                    ? template.isGradient
                    : prev.isGradient,
              }));
