  ...
  const renderContent = () => {
    return (
      <div className="relative w-full h-full">
        {currentWorkflow?.tools.map((toolItem, index) => (
          <div
            key={toolItem.id}
            style={{
              position: 'absolute',
              ...
              width: '100%',
              height: '100%',
              opacity: toolItem.id === tool.id ? 1 : 0,
              pointerEvents: toolItem.id === tool.id ? 'auto' : 'none',
              transition: 'opacity 0.3s ease-in-out',
              zIndex: toolItem.id === tool.id ? 1 : 0,
              visibility: toolItem.id === tool.id ? 'visible' : 'hidden'
            }}
          >
            <ToolRenderer tool={toolItem} />
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div className="w-full h-full flex flex-col items-center">
      <div className="p-4 pt-0  min-w-[600px] max-w-full">
        <Stepper
          value={currentWorkflow?.tools.findIndex(tItem => tItem.id === tool.id)! + 1}
            ...
            >
              <StepperTrigger className="max-md:flex-col" onClick={(e) => {
                ...
                <div className="text-center md:text-left">
                  <StepperTitle>{t(toolItem.name)}</StepperTitle>
                </div>
              </StepperTrigger>
